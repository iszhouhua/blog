<template>
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :default-sort = "{prop: 'id', order: 'descending'}" :data="list" border fit highlight-current-row style="width: 100%" @sort-change="sortChange">
      ...
      <el-table-column align="center" label="分类" min-width="100" prop="name"/>

      <el-table-column align="center" label="分类链接" min-width="100">
        <template slot-scope="scope">
          <a :href="BLOG_URL+'category/'+scope.row.url+'/'" style="color: #337ab7;" target="_blank">{{ scope.row.url }}</a>
        </template>
      </el-table-column>
      ...
      <el-table-column width="200" align="center" label="操作">
        <template slot-scope="scope">
          ...
          <el-button size="mini" icon="el-icon-delete" style="margin-left: 10px;" type="danger" @click="removeCategory(scope.row.id)">删除
          </el-button>
        </template>
      </el-table-column>
    </el-table>
